...
if (process.env.STRIPE_SECRET_KEY) {
  ...
  console.warn(
    "⚠️  STRIPE_SECRET_KEY no está configurada. Los endpoints de pago no funcionarán."
  );
  stripe = null;
}
...
const PostCheckoutSession = async (req, res) => {
  ...
  if (!stripe) {
    return res.status(503).json({
      ...
      details: "Stripe no está configurado en el servidor",
    });
  }
  ...
  try {
    ...
    if (error.code === "ECONNRESET") {
      return res.status(503).json({
        ...
        details:
          "No se pudo establecer conexión con el servidor de pagos. Por favor, intente nuevamente.",
      });
    } else if (error.type === "StripeConnectionError") {
      return res.status(503).json({
        error: "Error de conexión con el servicio de pagos",
        details:
          "No se pudo conectar con el servicio de pagos. Por favor, verifique su conexión a internet e intente nuevamente.",
      });
    }
...
const postCustomer = async (req, res) => {
  ...
  try {
    console.log("Creando cliente en Stripe con datos:", {
      email,
      nombre,
      userId,
    });
...
const getSuscriptionById = async (req, res) => {
  ...
  try {
    ...
    const response = {
      ...
      billing_cycle_anchor_formatted: formatDate(
